...
| **controller** | Webhook receiver and index reconciler for bucket notifications | 8080 |
...
| `RECONCILE_INTERVAL_SECONDS` | `10` | Delay between controller reconcile passes |
...
Events generated by the controller's own writes under `metadata/`, `indexes/`
and `manifest.yaml` are ignored, so reconciliation does not loop on itself.
